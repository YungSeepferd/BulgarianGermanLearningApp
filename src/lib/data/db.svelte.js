...
export class VocabularyDB {
    ...
    getVocabularyByDifficulty(difficulty) {
        ...
        return this.items.filter(item => item.difficulty >= difficultyRange.min && item.difficulty <= difficultyRange.max);
    }
    /**
     ...
     */
    getVocabularyByPartOfSpeech(partOfSpeech) {
    ...
    searchVocabulary(query) {
        ...
        return this.items.filter(item => item.german.toLowerCase().includes(lowerQuery) ||
     ...
     */
    getVocabularyStatsByDifficulty() {
        const stats = {
            'A1': 0,
            'A2': 0,
            'B1': 0,
        ...
        this.items.forEach(item => {
            if (item.difficulty <= 1.5)
                stats.A1++;
            else if (item.difficulty <= 2.5)
                stats.A2++;
            else if (item.difficulty <= 3.5)
                stats.B1++;
            else if (item.difficulty <= 4.5)
                stats.B2++;
            else
                stats.C1++;
        });
...
//# sourceMappingURL=db.svelte.js.map