...
class ProfileManager {
  private PROFILES_KEY: string = 'bgde:profiles';
  private ACTIVE_PROFILE_KEY: string = 'bgde:active_profile';
  private PROFILE_IDS = {
  ...
  addCustomVocabulary(profileId: string, vocabEntry: Omit<CustomVocabularyEntry, 'id' | 'addedAt' | 'source' | 'enrichmentStatus'>): boolean {
    ...
    const entry: CustomVocabularyEntry = {
      id: vocabId,
      ...vocabEntry,
   ...
   */
  private calculateEnrichmentStatus(vocabEntry: Omit<CustomVocabularyEntry, 'id' | 'addedAt' | 'source' | 'enrichmentStatus'>): 'complete' | 'partial' | 'minimal' {
    let score = 0;
   ...
   */
  getProfileData<T>(key: string, profileId: string | null = null): T | null {
   ...
   */
  setProfileData<T>(key: string, value: T, profileId: string | null = null): void {
    const namespacedKey = this.getNamespacedKey(key, profileId);
  ...
  importProfile(profileId: string, importData: Record<string, unknown>): boolean {
    ...
    if (importData.profile && typeof importData.profile === 'object') {
      this.updateProfile(profileId, importData.profile as Partial<Profile>);
    }
