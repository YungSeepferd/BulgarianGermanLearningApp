 ...
 */
const CORE_VOCABULARY_CHECKLIST: Array<{
  german: string;
  bulgarian: string;
  partOfSpeech: string;
  categories: string[];
...
/**
 * Completeness report for core vocabulary
 */
export interface CoreVocabularyReport {
  ...
  missingByLevel: Record<string, number>;
  timestamp: string;
}
 ...
 * Core vocabulary completeness checker
 */
export class CoreVocabularyChecker {
  private existingVocab: VocabularyItem[];
  private existingTerms: Set<string> = new Set();
  private existingBulgarianTerms: Set<string> = new Set();
   ...
   */
  checkCompleteness(): CoreVocabularyReport {
   ...
   */
  generateReport(report: CoreVocabularyReport): string {
    ...
    if (report.missingEntries.length > 0) {
      ...
      const byLevel = report.missingEntries.reduce((acc, entry) => {
        ...
        return acc;
      }, {} as Record<string, typeof report.missingEntries>);
   ...
   */
  generateMissingEntriesTemplate(missingEntries: Array<{
    german: string;
    bulgarian: string;
    partOfSpeech: string;
    categories: string[];
...
export default CoreVocabularyChecker;