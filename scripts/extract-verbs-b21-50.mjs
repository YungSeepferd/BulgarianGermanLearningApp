import fs from 'fs';

const BATCH_START = 200;
const BATCH_END = 500;

const items = (JSON.parse(fs.readFileSync('data/unified-vocabulary.json', 'utf8')).items) || [];
const batchItems = items.slice(BATCH_START, BATCH_END);

const verbs = batchItems.filter(it => it.partOfSpeech === 'verb');

const outputPath = 'enrichment-output/verbs-b21-50-list.json';
fs.writeFileSync(outputPath, JSON.stringify(verbs.map(v => ({
    id: v.id,
    german: v.german,
    cleanGerman: v.german.split(',')[0].split('(')[0].trim() // Handle "fahren (weggehen)" -> "fahren"
})), null, 2));

console.log(`Found ${verbs.length} verbs in batches 21-50.`);
console.log(`Saved list to ${outputPath}`);
