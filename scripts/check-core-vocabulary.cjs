 ...
 *
 * Usage:
 * node scripts/check-core-vocabulary.js
 */

const fs = require('fs');
const path = require('path');
const { CoreVocabularyChecker } = require('./enrichment/core-vocabulary-checker.cjs');

function checkCoreVocabulary() {
  try {
    console.log('🔍 Checking core vocabulary completeness...');

    // Load vocabulary data
    const vocabPath = path.join(__dirname, '../src/lib/data/unified-vocabulary.json');
    ...
    const reportPath = path.join(__dirname, '../reports/core-vocabulary-completeness.md');
    ...
    if (report.missingEntries.length > 0) {
      ...
      const templatePath = path.join(__dirname, '../reports/missing-core-vocabulary-template.json');
