...
const out = path.join(__dirname, '../enrichment-output/nouns-b1-20-missing-gender.json');
fs.writeFileSync(out, JSON.stringify({ count: nouns.length, items: nouns }, null, 2));

console.log(`Found ${nouns.length} nouns missing gender in batches 1-20`);
