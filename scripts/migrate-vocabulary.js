...
async function loadMainVocabulary() {
...
async function loadSpecializedVocabulary() {
...
function deduplicateAndMerge(items) {
    ...
    for (const item of items) {
        ...
        if (itemMap.has(key)) {
            ...
            // Update timestamps
            existingItem.createdAt = new Date(Math.min(new Date(existingItem.createdAt).getTime(), new Date(normalizedItem.createdAt).getTime()));
            existingItem.updatedAt = new Date(Math.max(new Date(existingItem.updatedAt).getTime(), new Date(normalizedItem.updatedAt).getTime()));
...
//# sourceMappingURL=migrate-vocabulary.js.map