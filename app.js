...
let currentDirection = LearningDirection.BG_TO_DE;

// Direction specific strings
const directionStrings = {
    [LearningDirection.BG_TO_DE]: {
        toggleLabel: 'BG→DE',
        appTitle1: 'Български',
        appTitle2: 'Учебно приложение',
        searchPlaceholder: 'Търсете думи...',
        emptyState: 'Няма резултати',
        partOfSpeech: 'Част на речта',
        notes: 'Бележки',
        examples: 'Примери',
        allLabel: 'Всички',
        vocabularyTitle: level => `${level} Лексика`,
        grammarTitle: level => `${level} Граматика`,
        wordDetailTitle: 'Детайли за думата',
        grammarDetailTitle: 'Граматическа тема',
        settings: {
            title: 'Настройки',
            audio: 'Аудио настройки',
            speechRate: 'Скорост на речта',
            volume: 'Сила на звука',
            rateDesc: 'Регулирайте скоростта на произношението.',
            volumeDesc: 'Регулирайте силата на произношението.',
            testTitle: 'Тест Аудио',
            testLabel: 'Тествайте произношение',
            aboutTitle: 'Относно',
            appName: 'Българо-немско приложение',
            version: 'Версия 1.0',
            aboutDesc: 'Учете български и немски с произношение.'
        }
    },
    [LearningDirection.DE_TO_BG]: {
        toggleLabel: 'DE→BG',
        appTitle1: 'Bulgarisch',
        appTitle2: 'Lern App',
        ...
        examples: 'Beispiele',
        allLabel: 'Alle',
        vocabularyTitle: level => `${level} Vokabeln`,
        grammarTitle: level => `${level} Grammatik`,
        wordDetailTitle: 'Wortdetails',
        grammarDetailTitle: 'Grammatikthema',
        settings: {
            title: 'Einstellungen',
            audio: 'Audioeinstellungen',
            speechRate: 'Sprechgeschwindigkeit',
            volume: 'Lautstärke',
            rateDesc: 'Stellen Sie ein, wie schnell gesprochen wird.',
            volumeDesc: 'Stellen Sie die Lautstärke ein.',
            testTitle: 'Audio testen',
            testLabel: 'Aussprache testen',
            aboutTitle: 'Über',
            appName: 'Bulgarisch-Deutsche Lern-App',
            version: 'Version 1.0',
            aboutDesc: 'Lerne Bulgarisch und Deutsch mit Aussprache.'
        }
    }
};

const partOfSpeechMap = {
    'Begrüßung': { bg: 'Поздрав', de: 'Begrüßung' },
    'Ausdruck': { bg: 'Израз', de: 'Ausdruck' },
    'Substantiv': { bg: 'Съществително', de: 'Substantiv' },
    'Verb': { bg: 'Глагол', de: 'Verb' },
    'Adjektiv': { bg: 'Прилагателно', de: 'Adjektiv' },
    'Adverb': { bg: 'Наречие', de: 'Adverb' },
    'Zahl': { bg: 'Число', de: 'Zahl' },
    'Quantor': { bg: 'Количествена дума', de: 'Quantor' },
    'Tag': { bg: 'Ден', de: 'Tag' }
};

function getDisplayWord(item) {
    return currentDirection === LearningDirection.BG_TO_DE ? item.word : item.translation;
}

function getDisplayTranslation(item) {
    return currentDirection === LearningDirection.BG_TO_DE ? item.translation : item.word;
}

function getTypeLabel(type) {
    const entry = partOfSpeechMap[type];
    if (!entry) return type;
    return currentDirection === LearningDirection.BG_TO_DE ? entry.bg : entry.de;
}
...
function showVocabulary(level) {
    ...
    document.getElementById('vocabulary-title').textContent = directionStrings[currentDirection].vocabularyTitle(level);
    showView('vocabulary');
...
function renderVocabularyList() {
    ...
    if (filteredItems.length === 0) {
        container.innerHTML = `<div class="empty-state">${directionStrings[currentDirection].emptyState}</div>`;
        return;
    }

    container.innerHTML = filteredItems.map(item => {
        const displayWord = getDisplayWord(item);
        const displayTranslation = getDisplayTranslation(item);
        return `
        <div class="vocabulary-item" onclick="showVocabularyDetail('${item.word}')">
            <div class="vocabulary-content">
                <div class="vocabulary-word">${displayWord}</div>
                <div class="vocabulary-translation">${displayTranslation}</div>
                <div class="vocabulary-type">${getTypeLabel(item.type)}</div>
            </div>
            <button class="play-btn" onclick="event.stopPropagation(); speakWord('${displayWord}')">
...
function renderFilterTabs() {
    ...
    container.innerHTML = types.map(type => {
        const label = type === 'All' ? directionStrings[currentDirection].allLabel : getTypeLabel(type);
        return `
...
function showVocabularyDetail(word) {
    ...
    currentWord = word;
    document.getElementById('detail-title').textContent = directionStrings[currentDirection].wordDetailTitle;
    ...
    content.innerHTML = `
        <div class="detail-word">
            ${displayWord}
            <button class="play-btn" onclick="speakWord('${displayWord}')">
                <i class="fas fa-play"></i>
            </button>
        </div>
        <div class="detail-translation">${displayTranslation}</div>
        <div class="detail-type">${directionStrings[currentDirection].partOfSpeech}: ${getTypeLabel(item.type)}</div>
        ${item.notes ? `
            <div class="detail-notes">
                <h4>${directionStrings[currentDirection].notes}:</h4>
                <p>${item.notes}</p>
            </div>
        ` : ''}
    `;
...
function showGrammar(level) {
    ...
    document.getElementById('grammar-title').textContent = directionStrings[currentDirection].grammarTitle(level);
    showView('grammar');
...
function showGrammarDetail(title) {
    ...
    if (!item) return;
    currentGrammarTitle = title;
    document.getElementById('grammar-detail-title').textContent = directionStrings[currentDirection].grammarDetailTitle;
    ...
    content.innerHTML = `
        ...
        <div class="grammar-examples">
            <h4>${directionStrings[currentDirection].examples}:</h4>
...
function toggleDirection() {
    currentDirection = currentDirection === LearningDirection.BG_TO_DE ? LearningDirection.DE_TO_BG : LearningDirection.BG_TO_DE;

    if (currentView === 'vocabulary') {
        renderVocabularyList();
        renderFilterTabs();
        document.getElementById('vocabulary-title').textContent = directionStrings[currentDirection].vocabularyTitle(currentLevel);
    } else if (currentView === 'vocabulary-detail' && currentWord) {
        showVocabularyDetail(currentWord);
    } else if (currentView === 'grammar') {
        renderGrammarList();
        document.getElementById('grammar-title').textContent = directionStrings[currentDirection].grammarTitle(currentLevel);
    } else if (currentView === 'grammar-detail' && currentGrammarTitle) {
        showGrammarDetail(currentGrammarTitle);
    }
...
function updateDirectionUI() {
    document.querySelectorAll('.direction-toggle').forEach(btn => {
        btn.textContent = directionStrings[currentDirection].toggleLabel;
        btn.setAttribute('aria-label', directionStrings[currentDirection].toggleLabel);
    });

    document.getElementById('app-title-1').textContent = directionStrings[currentDirection].appTitle1;
    document.getElementById('app-title-2').textContent = directionStrings[currentDirection].appTitle2;

    const searchInput = document.getElementById('search-input');
    if (searchInput) {
        searchInput.placeholder = directionStrings[currentDirection].searchPlaceholder;
    }

    const s = directionStrings[currentDirection].settings;
    document.getElementById('settings-title').textContent = s.title;
    document.getElementById('audio-settings-title').textContent = s.audio;
    const rateLabel = document.getElementById('speech-rate-label');
    if (rateLabel && rateLabel.childNodes[0]) {
        rateLabel.childNodes[0].textContent = s.speechRate + ': ';
    }
    document.getElementById('speech-rate-desc').textContent = s.rateDesc;
    const volumeLabel = document.getElementById('speech-volume-label');
    if (volumeLabel && volumeLabel.childNodes[0]) {
        volumeLabel.childNodes[0].textContent = s.volume + ': ';
    }
    document.getElementById('speech-volume-desc').textContent = s.volumeDesc;
    document.getElementById('test-audio-title').textContent = s.testTitle;
    document.getElementById('test-audio-label').textContent = s.testLabel;
    document.getElementById('about-title').textContent = s.aboutTitle;
    document.getElementById('about-app-name').textContent = s.appName;
    document.getElementById('about-version').textContent = s.version;
    document.getElementById('about-description').textContent = s.aboutDesc;
}
...
function speakWord(word) {
    ...
    utterance.lang = currentDirection === LearningDirection.BG_TO_DE ? 'bg-BG' : 'de-DE';
