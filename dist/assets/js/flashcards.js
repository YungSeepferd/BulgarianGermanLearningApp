...
export class Flashcards {
    ...
    handleSpeechResult(transcript) {
        ...
        const match = heard.includes(expected) ||
    ...
    async prepareSession() {
        ...
        if (this.mode === 'due') {
            ...
            filteredData = filteredData.filter(item => dueItems.some(dueItem => dueItem.itemId === this.generateWordId(item.word)));
        }
    ...
    updateCardContent() {
        ...
        if (this.flashcard) {
            this.flashcard.setAttribute('aria-label', `Flashcard ${this.currentIndex + 1} of ${this.sessionStats.totalCards}: ${frontText}`);
        }
    }
    getCardText(vocab) {
    ...
    reviewMistakes() {
        ...
        this.sessionCards = mistakeIndices.map(index => this.sessionCards[index]).filter((item) => item !== undefined);
