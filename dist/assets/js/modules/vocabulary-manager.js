 ...
 * @since 1.0.0
 */
import { ErrorFactory, ValidationError, StorageError, ParsingError, RetryStrategy } from './error-types.js';
...
export class VocabularyManager {
    ...
    loadCustomVocabulary() {
        ...
        return this.retryStrategy.recover(new StorageError('Loading custom vocabulary', 'read', this.storageKey), { retryFunction: async () => {
                try {
                    ...
                    if (!Array.isArray(parsed)) {
                        throw new ParsingError('Custom vocabulary data is not an array', 'JSON', data, 'array of vocabulary entries');
                    }
                    // Validate each entry
            ...
            errorLogger.error('Failed to load custom vocabulary after retries', error instanceof Error ? error : undefined, {
                storageKey: this.storageKey
            }, 'vocabulary-manager');
            return [];
        });
    ...
    async saveCustomVocabulary() {
        try {
            ...
            if (!validation.isValid) {
                throw new ValidationError('Cannot save invalid vocabulary data', undefined, this.customVocabulary, 'valid vocabulary array', validation.errors.join(', '));
            }
            ...
            if (size > availableSpace) {
                throw new StorageError('Vocabulary data too large for localStorage', 'write', this.storageKey, availableSpace, size);
            }
            // Attempt to save with retry logic
            return await this.retryStrategy.recover(new StorageError('Saving custom vocabulary', 'write', this.storageKey), { retryFunction: async () => {
                    try {
     ...
     */
    validateVocabularyArray(entries) {
        const errors = [];
    ...
    async createEntry(data) {
        try {
            ...
            const isDuplicate = this.customVocabulary.some(existing => existing.bulgarian.toLowerCase() === entry.bulgarian.toLowerCase() &&
    ...
    async updateEntry(id, updates) {
        try {
            ...
            const isDuplicate = this.customVocabulary.some((existing, existingIndex) => existingIndex !== index &&
    ...
    async importFromJSON(jsonData) {
        try {
            ...
            try {
                ...
                throw new ParsingError(`JSON parse error: ${parseError.message}`, 'JSON', jsonData.slice(0, 500), 'array of vocabulary entries');
            }
            if (!Array.isArray(entries)) {
                return {
                    success: false,
