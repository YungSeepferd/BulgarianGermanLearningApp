...
class PracticePageModule {
    ...
    async getSessionItems() {
        ...
        sessionItems =
            this.selectedItems.length > 0
                ? availableItems.filter((item) => this.selectedItems.includes(item.word) || this.selectedItems.includes(item.id))
    ...
    selectOptimalItems(availableItems, dueItems) {
        ...
        const dueVocabItems = availableItems.filter((item) => dueItemIds.has(item.id));
...
//# sourceMappingURL=practice-page.js.map