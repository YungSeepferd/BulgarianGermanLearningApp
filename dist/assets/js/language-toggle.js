...
class LanguageToggle {
    ...
    toggleDirection() {
        const nextDirection = this.currentDirection === DIRECTION.DE_TO_BG ? DIRECTION.BG_TO_DE : DIRECTION.DE_TO_BG;
        // Show confirmation modal if available
        const LanguageToggleConfirmation = window.LanguageToggleConfirmation;
        if (LanguageToggleConfirmation === undefined) {
            ...
            confirmation.show(this.currentDirection, nextDirection, (confirmedDirection) => {
    ...
    updateMetaDescription() {
        const descriptions = {
            [DIRECTION.DE_TO_BG]: 'Bulgarisch lernen für Deutsche - Interaktive Vokabeln und Grammatik mit Spaced Repetition',
            [DIRECTION.BG_TO_DE]: 'Учене на немски за българи - Интерактивни думи и граматика със система за повторение'
        };
    ...
    migrateLegacyDirection(explicitValue) {
        try {
            const legacyValue = explicitValue === undefined ? localStorage.getItem(this.legacyStorageKey) : explicitValue;
            if (!legacyValue) {
                return null;
            }
