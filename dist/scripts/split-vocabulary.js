...
class VocabularySplitter {
    constructor() {
        this.vocabulary = [];
        this.splitResults = [];
        this.maxFileSizeKB = 50; // Target maximum file size
    ...
    async saveSplitFile(fileName, entries, description) {
    ...
    async splitLargeCategory(baseFileName, entries, description) {
    ...
    async saveSplitFileDirect(fileName, entries, description) {
        const filePath = join(this.vocabDir, fileName);
        const content = JSON.stringify(entries, null, 2);
        const sizeKB = Math.round(Buffer.byteLength(content, 'utf8') / 1024);
        await writeFile(filePath, content);
    ...
    validateEntrySchema(entry) {
        ...
        if (!entry.translation || typeof entry.translation !== 'string')
            return false;
        if (!entry.source_lang || typeof entry.source_lang !== 'string')
            return false;
        if (!entry.target_lang || typeof entry.target_lang !== 'string')
            return false;
        ...
        if (entry.level && typeof entry.level !== 'string')
            return false;
        if (entry.difficulty && (typeof entry.difficulty !== 'number' || entry.difficulty < 1 || entry.difficulty > 5))
            return false;
        if (entry.frequency && (typeof entry.frequency !== 'number' || entry.frequency < 0 || entry.frequency > 100))
            return false;
        // Examples validation
...
export { VocabularySplitter };
//# sourceMappingURL=split-vocabulary.js.map