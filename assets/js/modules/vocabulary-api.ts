...
import { loadingStateManager, LoadingEvent } from './loading-state-manager.js';
...
class VocabularyAPI {
    ...
    try {
      ...
      if (!response.ok) {
        throw ErrorFactory.network(
          `Failed to load vocabulary index: ${response.statusText}`,
      ...
      if (!this.validateIndex(indexData)) {
        throw new ParsingError(
          'Invalid vocabulary index structure',
          'JSON',
          JSON.stringify(indexData).slice(0, 500),
          'valid VocabularyIndex structure'
        );
      }
      ...
      try {
        ...
        if (cachedIndex) {
          this.index = cachedIndex as VocabularyIndex;
  ...
  private initializeEventListeners(): void {
    loadingStateManager.subscribe((event: LoadingEvent) => {
   ...
   */
  private handleLoadingEvent(event: LoadingEvent): void {
    ...
    if (event.type === 'loading-failed' && event.error) {
      ...
      errorLogger.logLoadingState(event.operationId, 'completed', (event.progress as any)?.percentage, {
        operationId: event.operationId,
        duration: (event.progress as any)?.duration
      });
    }
  }
   ...
   */
  private validateIndex(index: unknown): index is VocabularyIndex {
    if (!index || typeof index !== 'object') return false;
    
    const idx = index as any;
    return (
      typeof idx.generated === 'string' &&
  ...
  async loadChunk(fileName: string): Promise<VocabularyEntry[]> {
    ...
    try {
      const memoryChunk = await memoryManager.get<VocabularyEntry[]>(`vocab-chunk-${fileName}`);
      if (memoryChunk) {
    ...
    return (this.retryStrategy.recover(
      new NetworkError(`Failed to load chunk ${fileName}`, undefined, `${this.basePath}${fileName}`),
      {
        retryFunction: async (): Promise<VocabularyEntry[]> => {
          try {
            ...
            if (!response.ok) {
              throw ErrorFactory.network(
                `Failed to load vocabulary chunk: ${response.statusText}`,
                { url: `${this.basePath}${fileName}` }
              );
            }

            const entries: VocabularyEntry[] = await response.json();
            ...
            if (validEntries.length === 0) {
              throw new ParsingError(
                `No valid entries found in ${fileName}`,
                'JSON',
                JSON.stringify(entries).slice(0, 500),
                'array with valid vocabulary entries'
              );
            }
  ...
  async loadByCategory(category: string): Promise<VocabularyEntry[]> {
    ...
    if (sanitizedCategory.length === 0) {
      throw new ValidationError(
        'Category contains no valid characters',
        'category',
        category,
        'category with valid characters'
      );
    }
  ...
  async loadAll(): Promise<VocabularyEntry[]> {
    ...
    const operationId = loadingStateManager.startLoading(
      this.index.splitFiles.length,
      'Loading all vocabulary chunks...'
    );
        ...
        try {
          ...
          if (failedChunks > this.index.splitFiles.length * 0.5) {
            throw new NetworkError(
              `Too many chunks failed to load (${failedChunks}/${this.index.splitFiles.length})`,
              undefined,
              undefined,
              undefined,
              0
      ...
      try {
        ...
        if (cachedAll) {
          errorLogger.info('Returning cached vocabulary as fallback', {
            entryCount: (cachedAll as VocabularyEntry[]).length
          }, 'vocabulary-api');
          
          return cachedAll as VocabularyEntry[];
        }
      } catch (cacheError) {
  ...
  async loadFiltered(filters: { level?: string; category?: string; search?: string } = {}): Promise<VocabularyEntry[]> {
    try {
      ...
      if (filters.level && !['A1', 'A2', 'B1', 'B2', 'C1', 'C2'].includes(filters.level)) {
        throw new ValidationError(
          `Invalid level filter: ${filters.level}`,
          'level',
          filters.level,
          'valid CEFR level'
        );
   ...
   */
  isLoading(): boolean {
    return (loadingStateManager as any).isLoading();
  }
  ...
  getProgress(): VocabularyLoadingState {
    const progress = (loadingStateManager as any).getCurrentProgress();
    return {
   ...
   */
  private extractLevelFromFileName(fileName: string): string {
