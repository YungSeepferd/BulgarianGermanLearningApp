...
class ProfileManager {
  ...
  addCustomVocabulary(profileId, vocabEntry) {
    ...
    const vocabId = `custom_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const entry = {
      id: vocabId,
      ...vocabEntry,
  ...
  calculateEnrichmentStatus(vocabEntry) {
    ...
    if (vocabEntry.bulgarian) score += 1;
    if (vocabEntry.german) score += 1;

    // Optional enrichment fields
    if (vocabEntry.category) score += 1;
    if (vocabEntry.level) score += 1;
    if (vocabEntry.examples && vocabEntry.examples.length > 0) score += 2;
    if (vocabEntry.notes) score += 1;
    if (vocabEntry.etymology) score += 1;
    if (vocabEntry.cultural_note) score += 1;
  ...
  getIncompleteVocabularyCount(profileId) {
    ...
    const customVocab = profile.customVocabulary || [];
    return customVocab.filter(v =>
      v.enrichmentStatus === 'minimal' || v.enrichmentStatus === 'partial'
    ).length;
  }
  ...
  getProfileComparison() {
    ...
    return {
      german_learner: {
        name: germanLearner.displayName,
        totalReviews: germanLearner.statistics.totalReviews,
        accuracy: germanLearner.statistics.totalReviews > 0
          ? (germanLearner.statistics.totalCorrect / germanLearner.statistics.totalReviews * 100).toFixed(1)
          : 0,
        streak: germanLearner.statistics.streak,
        lastAccessed: germanLearner.lastAccessedAt
      },
      bulgarian_learner: {
        name: bulgarianLearner.displayName,
        totalReviews: bulgarianLearner.statistics.totalReviews,
        accuracy: bulgarianLearner.statistics.totalReviews > 0
          ? (bulgarianLearner.statistics.totalCorrect / bulgarianLearner.statistics.totalReviews * 100).toFixed(1)
          : 0,
        streak: bulgarianLearner.statistics.streak,
        lastAccessed: bulgarianLearner.lastAccessedAt
      }
    };
  }
  ...
  resetAllProfiles() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      const key = localStorage.key(i);
      if (key.startsWith('bgde:german_learner:') || key.startsWith('bgde:bulgarian_learner:')) {
