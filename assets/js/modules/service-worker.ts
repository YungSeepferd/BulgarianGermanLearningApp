 ...
 *
 * @typedef {string} ServiceWorkerMessageType
 *
 * @example
 * ```typescript
 ...
 * ```
 */
type ServiceWorkerMessageType = 'SW_ACTIVATED' | 'CACHE_UPDATED' | 'SKIP_WAITING' | 'CLEAR_CACHE' | 'GET_VERSION' | 'PREFETCH_VOCABULARY';

/**
 * Service worker message interface for bidirectional communication
 *
 * @interface ServiceWorkerMessage
 * @property {ServiceWorkerMessageType} type - Message type identifier
 * @property {string} [version] - Service worker version (for activation messages)
 * @property {string} [url] - URL associated with the message (for cache updates)
 * @property {any} [data] - Additional data payload
 *
 * @example
 * ```typescript
 * const message: ServiceWorkerMessage = {
...
/**
 * Before install prompt event interface for PWA installation
 *
 * This interface extends the standard Event interface to provide
 * PWA installation capabilities through the beforeinstallprompt event.
 *
 * @interface BeforeInstallPromptEvent
 * @extends Event
 * @property {Function} prompt - Shows the install prompt to the user
 * @property {Promise<{outcome: 'accepted'|'dismissed'}>} userChoice - Promise resolving to user's choice
 *
 * @example
 * ```typescript
...
class ServiceWorkerManager {
  ...
  private periodicUpdateTimer: number | null;
  ...
  private setupEventListeners(): void {
    ...
    navigator.serviceWorker.addEventListener('message', (event: MessageEvent) => {
      this.handleServiceWorkerMessage(event.data as ServiceWorkerMessage);
    });
    ...
    window.addEventListener('beforeinstallprompt', (event: Event) => {
      ...
      this.showInstallPrompt(event as BeforeInstallPromptEvent);
    });
  ...
  private async applyUpdate(): Promise<void> {
    ...
    this.swRegistration.waiting.postMessage({ type: 'SKIP_WAITING' } as ServiceWorkerMessage);
  ...
  private showConnectionNotification(isOnline: boolean): void {
    // Remove existing notification
  ...
  private async prefetchVocabularyData(): Promise<void> {
    try {
  ...
  public async clearCache(): Promise<boolean> {
    ...
    return new Promise((resolve) => {
      ...
      this.swRegistration!.active!.postMessage(
        { type: 'CLEAR_CACHE' } as ServiceWorkerMessage,
