...
export class LoadingIntegration {
  private options: Required<LoadingIntegrationOptions>;
  private activeLoadings: Map<string, string> = new Map(); // operationId -> componentId
   ...
   */
  async loadVocabularyWithUI(level?: string, category?: string): Promise<any[]> {
    const operationId = `load-vocab-${Date.now()}`;
    let modalElement: HTMLElement | null = null;
    let progressBarComponent: any = null;
    ...
    try {
      ...
      if (this.options.showToasts) {
        loadingUIManager.showToast(
          `Loading vocabulary${level ? ` (${level})` : ''}${category ? ` - ${category}` : ''}...`,
          'info',
          5000
        );
      }
      ...
      if (this.options.showProgressBars) {
        progressBarComponent = loadingUIManager.createComponent(
          operationId,
          'progress-bar',
          document.querySelector('#loading-container') || undefined
        );
      ...
      if (this.options.showModals) {
        modalElement = loadingUIManager.showModal(
          `Loading vocabulary${level ? ` (${level})` : ''}${category ? ` - ${category}` : ''}...`,
          {
            showProgress: true,
            allowCancel: true,
      ...
      if (this.options.showToasts) {
        loadingUIManager.showToast(
          `Loaded ${vocabulary.length} vocabulary entries successfully`,
          'success',
          3000
        );
      }
      ...
      errorLogger.logVocabularyError(
        ErrorFactory.fromError(error, {
          operationId,
          level,
          category
        }),
        'loadVocabularyWithUI',
        {
          operationId,
          level,
          category
   ...
   */
  async preloadChunksWithUI(chunkNames: string[]): Promise<void> {
    ...
    try {
      ...
      if (this.options.showToasts) {
        loadingUIManager.showToast(
          `Preloading ${chunkNames.length} vocabulary chunks...`,
          'info',
          5000
        );
      }
      ...
      if (this.options.showToasts) {
        loadingUIManager.showToast(
          `Preloaded ${chunkNames.length} chunks successfully`,
          'success',
          3000
        );
      }
      ...
      errorLogger.error('Chunk preloading failed with UI', ErrorFactory.fromError(error, {
        operationId,
        chunkCount: chunkNames.length
      }), {
        operationId,
      ...
      if (this.options.showToasts) {
        loadingUIManager.showToast(
          `Failed to preload chunks: ${error instanceof Error ? error.message : 'Unknown error'}`,
          'error',
          5000
        );
      }
   ...
   */
  async getStatisticsWithUI(): Promise<any> {
    const operationId = `get-stats-${Date.now()}`;
    let spinnerComponent: any = null;
    ...
    try {
      ...
      if (this.options.showProgressBars) {
        spinnerComponent = loadingUIManager.createComponent(
          operationId,
          'spinner',
          document.querySelector('#loading-container') || undefined
        );
      ...
      errorLogger.error('Failed to get statistics with UI', ErrorFactory.fromError(error, {
        operationId
      }), {
        operationId
      }, 'loading-integration');
   ...
   */
  async clearCacheWithUI(): Promise<void> {
    ...
    try {
      ...
      errorLogger.error('Failed to clear cache with UI', ErrorFactory.fromError(error, {
        operationId
      }), {
        operationId
      }, 'loading-integration');
  ...
  cancelLoading(operationId: string): boolean {
    ...
    try {
      ...
      errorLogger.error('Failed to cancel loading operation', ErrorFactory.fromError(error, {
        operationId
      }), {
        operationId
      }, 'loading-integration');
  ...
  destroy(): void {
    try {
      ...
      errorLogger.error('Error during LoadingIntegration cleanup', ErrorFactory.fromError(error, {}), {}, 'loading-integration');
    }
  }
  ...
  private initializeEventListeners(): void {
    ...
    loadingUIManager.subscribe('component-created', (event: any) => {
    ...
    loadingUIManager.subscribe('component-updated', (event: any) => {
    ...
    loadingUIManager.subscribe('ui-error', (event: any) => {
