...
class SessionStatsDashboard {
  private spacedRepetition: SessionStatsDashboardOptions['spacedRepetition'];
  ...
  getReviewData(cutoffDate: Date | null): ReviewStats {
    ...
    const filtered = cutoffDate
      ? reviews.filter((r: ReviewState) => new Date(r.lastReview || 0) >= cutoffDate)
      : reviews;
            
    return {
      total: filtered.length,
      correct: filtered.filter((r: ReviewState) => r.repetitions > 0).length
    };
  }
  ...
  updateDifficultWords(): void {
    ...
    const difficultWords = reviewStates
      .filter((state: ReviewState) => state.easeFactor < 2 && state.repetitions > 2)
      .sort((a: ReviewState, b: ReviewState) => a.easeFactor - b.easeFactor)
      .slice(0, 10);
    ...
    container.innerHTML = difficultWords.map((state: ReviewState) => `
  ...
  updateMasteredWords(): void {
    ...
    const masteredWords = reviewStates
      .filter((state: ReviewState) => state.interval >= 30 && state.repetitions >= 5)
      .sort((a: ReviewState, b: ReviewState) => b.interval - a.interval)
      .slice(0, 10);
    ...
    container.innerHTML = masteredWords.map((state: ReviewState) => `
...
if (typeof window !== 'undefined') {
  (window as unknown as { SessionStatsDashboard: typeof SessionStatsDashboard }).SessionStatsDashboard = SessionStatsDashboard;
}
